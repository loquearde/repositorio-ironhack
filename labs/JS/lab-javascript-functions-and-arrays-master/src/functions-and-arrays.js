...
function findLongestWord(words) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    else if (words[i].length > theLongestWord.length) {
...
function sum(arraySum) {
  ...
  arraySum.forEach((element) => {
    ...
    if (typeof element === "number") result += element.length;
...
function averageNumbers(sum, numbersAvg) {
  let sumResult = 0;
  for (let i = 0; i < numbersAvg.length; i++) {
    sumResult += numbersAvg[i];
  }
  return sumResult / numbersAvg.length;
}
...
const resultArray = uniquifyArray(wordsUnique);
...
function doesWordExist(wordsArray, wordSearch) {
  ...
  const doesResultExist = result ? true : false;
...
function howManyTimes(newWordsArray, wordRepetition) {
  let counter = 0;
  newWordsArray.forEach((word) => {
    if (word === wordRepetition) {
      counter += 1;
    }
  });
  return counter;
